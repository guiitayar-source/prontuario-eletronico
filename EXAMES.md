...
Na opção OpenAI Luna, o padrão para rascunhos e leitura de exames é `gpt-6-luna` com `reasoning.effort: medium`. As variáveis `OPENAI_DOCUMENT_MODEL` e `OPENAI_LUNA_MODEL` podem substituir esse padrão; se estiverem configuradas com um modelo anterior no servidor, atualize-as para `gpt-6-luna`. `OPENAI_EXAM_MODEL` permanece para integrações legadas sem escolha de modelo.
